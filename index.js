...
con.connect((err) => {
    ...
    con.query("INSERT INTO Cliente (nome, idade, uf) VALUES ('Wellington', 18, 'SP')", (err, result) => {
        if (err) throw err;
        console.log("CLIENTE Wellington CRIADO COM SUCESSO!");
    });

    con.query("INSERT INTO Cliente (nome, idade, uf) VALUES ('Gabriel', 19, 'SP')", (err, result) => {
        if (err) throw err;
    ...
    // CADASTRANDO COMPRAS
    con.query("INSERT INTO Compra (nome_produto, data_compra, qtd_produto, id_cliente) VALUES ('PROCESSADOR INTEL 7 10GEN', CURDATE(), 1, 1)", (err, result) => {
        if (err) throw err;
        console.log("COMPRA CRIADA COM SUCESSO!");
    });

    con.query("INSERT INTO Compra (nome_produto, data_compra, qtd_produto, id_cliente) VALUES ('NVIDIA GTX 1660 SUPER', CURDATE(), 1, 1)", (err, result) => {
        if (err) throw err;
        console.log("COMPRA CRIADA COM SUCESSO!");
    });

    // UPDATE NO NOME DO CLIENTE DE ID = 1
    con.query("UPDATE Cliente set nome = 'Wellington Luiz de Faria' WHERE id = 1", (err, result) => {
        if (err) throw err;
        console.log("UPDATE DO NOME PARA Wellington Luiz de Faria DO CLIENTE DE ID = 1 REALIZADO COM SUCESSO!");
    });
    ...
    con.query("SELECT Cliente.nome, Compra.nome_produto, Compra.qtd_produto FROM Cliente INNER JOIN Compra ON Cliente.id = Compra.id_cliente", (err, result) => {
        if (err) throw err;
